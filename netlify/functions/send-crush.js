...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Email gönderme - Environment Variables'dan al
    const transporter = nodemailer.createTransport({
      service: 'gmail',
      auth: {
        user: process.env.EMAIL_USER, // Netlify'da ayarlanacak
        pass: process.env.EMAIL_PASS  // Netlify'da ayarlanacak
      }
    });

    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: process.env.MAIL_TO, // Hedef email (Netlify'da ayarlanacak)
    ...
    // Firebase'e kaydet (opsiyonel - Firebase Admin SDK kullanarak)
    // Eğer Firebase'e de kaydetmek istersen Firebase Admin SDK'yı kullanabilirsin
